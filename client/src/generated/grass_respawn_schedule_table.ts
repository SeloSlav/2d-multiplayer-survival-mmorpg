// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

/* eslint-disable */
/* tslint:disable */
// @ts-nocheck
import {
  AlgebraicType,
  AlgebraicValue,
  BinaryReader,
  BinaryWriter,
  CallReducerFlags,
  ConnectionId,
  DbConnectionBuilder,
  DbConnectionImpl,
  DbContext,
  ErrorContextInterface,
  Event,
  EventContextInterface,
  Identity,
  ProductType,
  ProductTypeElement,
  ReducerEventContextInterface,
  SubscriptionBuilderImpl,
  SubscriptionEventContextInterface,
  SumType,
  SumTypeVariant,
  TableCache,
  TimeDuration,
  Timestamp,
  deepEqual,
} from "@clockworklabs/spacetimedb-sdk";
import { GrassRespawnSchedule } from "./grass_respawn_schedule_type";
import { GrassRespawnData as __GrassRespawnData } from "./grass_respawn_data_type";

import { EventContext, Reducer, RemoteReducers, RemoteTables } from ".";

/**
 * Table handle for the table `grass_respawn_schedule`.
 *
 * Obtain a handle from the [`grassRespawnSchedule`] property on [`RemoteTables`],
 * like `ctx.db.grassRespawnSchedule`.
 *
 * Users are encouraged not to explicitly reference this type,
 * but to directly chain method calls,
 * like `ctx.db.grassRespawnSchedule.on_insert(...)`.
 */
export class GrassRespawnScheduleTableHandle {
  tableCache: TableCache<GrassRespawnSchedule>;

  constructor(tableCache: TableCache<GrassRespawnSchedule>) {
    this.tableCache = tableCache;
  }

  count(): number {
    return this.tableCache.count();
  }

  iter(): Iterable<GrassRespawnSchedule> {
    return this.tableCache.iter();
  }
  /**
   * Access to the `schedule_id` unique index on the table `grass_respawn_schedule`,
   * which allows point queries on the field of the same name
   * via the [`GrassRespawnScheduleScheduleIdUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.grassRespawnSchedule.schedule_id().find(...)`.
   *
   * Get a handle on the `schedule_id` unique index on the table `grass_respawn_schedule`.
   */
  schedule_id = {
    // Find the subscribed row whose `schedule_id` column value is equal to `col_val`,
    // if such a row is present in the client cache.
    find: (col_val: bigint): GrassRespawnSchedule | undefined => {
      for (let row of this.tableCache.iter()) {
        if (deepEqual(row.schedule_id, col_val)) {
          return row;
        }
      }
    },
  };

  onInsert = (cb: (ctx: EventContext, row: GrassRespawnSchedule) => void) => {
    return this.tableCache.onInsert(cb);
  }

  removeOnInsert = (cb: (ctx: EventContext, row: GrassRespawnSchedule) => void) => {
    return this.tableCache.removeOnInsert(cb);
  }

  onDelete = (cb: (ctx: EventContext, row: GrassRespawnSchedule) => void) => {
    return this.tableCache.onDelete(cb);
  }

  removeOnDelete = (cb: (ctx: EventContext, row: GrassRespawnSchedule) => void) => {
    return this.tableCache.removeOnDelete(cb);
  }

  // Updates are only defined for tables with primary keys.
  onUpdate = (cb: (ctx: EventContext, oldRow: GrassRespawnSchedule, newRow: GrassRespawnSchedule) => void) => {
    return this.tableCache.onUpdate(cb);
  }

  removeOnUpdate = (cb: (ctx: EventContext, onRow: GrassRespawnSchedule, newRow: GrassRespawnSchedule) => void) => {
    return this.tableCache.removeOnUpdate(cb);
  }}
